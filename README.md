...
# Introduce
基于CKEDITOR默认内置的 [Image](http://ckeditor.com/addon/image) 插件修改而来，本着尽量少修改代码的原则，只添加了上传到 [又拍云](http://upyun.com) 功能，未修改 image 插件的任何代码逻辑。插件提供了两种上传方式，REST API和FORM API，基于REST API上传方式使用了又拍云官方提供的 SDK [`js-multipart-upload`](https://github.com/upyun/js-multipart-upload))。
...
CKEDITOR.config.imageuploadforupyun = {
    upload_type: 'rest_api',                                       // 若使用REST API方式上传，此项为必填项，不填或值不是'rest_api'的话，将采用基于FORM API
